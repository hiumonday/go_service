...
func (h *ImportHandler) ImportUsers(c *gin.Context) {
	...
	if !validateHeaders(csvHeaders, expectedHeaders) {
		...
		c.JSON(http.StatusBadRequest, gin.H{
			"error": fmt.Sprintf("Invalid CSV headers, expected: %v", expectedHeaders),
		})
		return
	}
	...
	for i := 0; i < maxWorkers; i++ {
		wg.Add(1)
		go h.worker(c.Request.Context(), jobs, results, &wg)
	}
...
func (h *ImportHandler) worker(ctx context.Context, jobs <-chan []string, results chan<- ImportResult, wg *sync.WaitGroup) {
	defer wg.Done()
